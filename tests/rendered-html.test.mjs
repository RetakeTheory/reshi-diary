...
test("ships Cloudflare-native OneBot with bounded scheduled reminders", async () => {
  const [worker, session, runtime, scheduler, authRoute, adminRoute, schema, login, manager, styles, docs, configText] = await Promise.all([
  ...
  assert.match(schema, /onebot_scheduled_messages/);
  ...
  assert.match(manager, /设置定时通知/);
  assert.doesNotMatch(manager, /Generated by reshi|reshi&(?:apos|#39);s diary/);
  ...
  assert.match(docs, /30秒后提醒我喝水/);
  assert.match(scheduler, /DELETE FROM onebot_scheduled_messages/);
