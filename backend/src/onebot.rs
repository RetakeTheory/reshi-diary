...
pub(crate) async fn complete_auth(
    ...
    if row.expires_at <= now_ms() {
        return Ok(status_json(StatusCode::GONE, serde_json::json!({"status":"expired","error":"验证已过期，请重新开始"})));
    }
    if row.status == "pending" {
        return Ok(status_json(StatusCode::ACCEPTED, serde_json::json!({"status":"pending"})));
    }
    if row.status == "failed" {
        return Ok(status_json(StatusCode::CONFLICT, serde_json::json!({"status":"failed","error":row.error.unwrap_or_else(|| "QQ 验证失败".into())})));
    }
    ...
    if user_id.is_none() {
        ...
        let name = row.display_name.unwrap_or_else(|| format!("QQ用户{}", &qq_id[qq_id.len().saturating_sub(4)..]));
