...
Axum backend for reshi-diary. It owns admin and reader auth, Passkeys, posts, attachments, comments, reactions and site notifications.
...
Build `backend/Dockerfile`, attach a persistent volume at `/app/data`, set `PUBLIC_ORIGIN` to the public frontend origin, then set the required frontend Worker variable `RUST_BACKEND_ORIGIN` to this service's HTTPS origin. API requests fail closed with HTTP 503 when this variable is absent.
