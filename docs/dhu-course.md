...
2. 在该会话中读取课程页引用的 `selecthome.js`，拒绝重定向到登录页或 HTML 响应。保存脚本 SHA-256、字节数、POST 调用线索、候选路径和字段线索；诊断日志只截取三个相关调用点附近的静态代码，不记录整份脚本、密码、验证码、Cookie、学生页面 HTML。`/appoint` 中的“重新读取学校提交脚本”按钮可重试此只读步骤，30 秒内至多一次。
