...
“同步 GitHub”会先确认工作区没有未提交文件，再获取 `origin/main` 并以快进方式更新整个本地项目。本机草稿会保留并在页面结构升级时自动迁移；未保存的画布修改、本地独有提交或真实分支冲突都会阻止同步，避免静默覆盖。

同步与发布由 localhost 进程调用本机 Git，因此电脑必须能连接 `github.com:443`；Codex 内置 GitHub 插件不能被普通 localhost 网页直接调用。网络不可达时编辑器会给出明确提示，连接可用网络或为 Git 配置代理后可直接重试。
