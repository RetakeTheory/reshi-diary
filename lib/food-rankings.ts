...
export function normalizeFoodRankingInput(raw: unknown) {
  ...
  return { listType, restaurant, location, category, summary, details, tags };
}
...
export function foodRankingFromRow(row: Record<string, unknown>): FoodRankingEntry {
  return {
    ...
    tags: JSON.parse(String(row.tagsJson || "[]")) as string[], createdAt: Number(row.createdAt), updatedAt: Number(row.updatedAt),
  };
}

export const foodRankingSelect = `SELECT id, list_type AS listType, restaurant, location, category, summary, details, tags_json AS tagsJson, created_at AS createdAt, updated_at AS updatedAt FROM food_rankings`;
