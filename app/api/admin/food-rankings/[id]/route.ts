...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await db.prepare("UPDATE food_rankings SET admin_rating = ?, restaurant = ?, location = ?, category = ?, summary = ?, details = ?, tags_json = ?, image_url = ?, latitude = ?, longitude = ?, updated_at = ? WHERE id = ?").bind(input.adminRating, input.restaurant, input.location, input.category, input.summary, input.details, JSON.stringify(input.tags), input.imageUrl, input.latitude, input.longitude, Date.now(), id).run();
