...
export async function POST(request: Request) {
  ...
  try {
    ...
    await ensureDatabaseSchema(); const db = await getD1();
    await db.prepare("INSERT INTO food_rankings (id, list_type, restaurant, location, category, summary, details, tags_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(id, input.listType, input.restaurant, input.location, input.category, input.summary, input.details, JSON.stringify(input.tags), now, now).run();
