...
export async function POST(request: Request) {
  ...
  try {
    ...
    await ensureDatabaseSchema(); const db = await getD1();
    // Keep the legacy storage value for databases with the old CHECK constraint; it is no longer exposed.
    await db.prepare("INSERT INTO food_rankings (id, list_type, admin_rating, restaurant, location, category, summary, details, tags_json, image_url, latitude, longitude, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(id, "red", input.adminRating, input.restaurant, input.location, input.category, input.summary, input.details, JSON.stringify(input.tags), input.imageUrl, input.latitude, input.longitude, now, now).run();
