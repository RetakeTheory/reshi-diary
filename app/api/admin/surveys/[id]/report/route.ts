...
type ResponseRow = { id: string; answersJson: string; createdAt: number };
...
export async function GET(_request: Request, context: Context) {
  ...
  const result = await db.prepare(`SELECT id, answers_json AS answersJson, created_at AS createdAt
    FROM survey_responses WHERE survey_id = ? ORDER BY created_at ASC`).bind(id).all<ResponseRow>();
  const survey = surveyFromRow(row);
  const csv = surveyResponsesCsv(survey, (result.results || []).map((item) => ({ id: item.id, answers: JSON.parse(item.answersJson) as SurveyAnswers, createdAt: item.createdAt })));
