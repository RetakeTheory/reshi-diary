...
export async function POST(request: Request) {
  ...
  const path = ({ startLogin: "/login/start", sendCode: "/login/code", finishLogin: "/login/finish",
    inspectMfa: "/login/inspect", openCoursePage: "/login/course", inspectProtocol: "/protocol/inspect", loadSections: "/course/sections",
    addTask: "/task", cancelTask: "/task/cancel" })[
    input?.action as "startLogin" | "sendCode" | "finishLogin" | "inspectMfa" | "openCoursePage" | "inspectProtocol" | "loadSections" | "addTask" | "cancelTask"];
