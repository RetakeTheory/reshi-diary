...
async function surveyForQuery(slug: string) {
  ...
  const row = await db.prepare(`${surveySelect} WHERE s.slug = ? AND s.query_enabled = 1 AND s.status IN ('published', 'closed') LIMIT 1`).bind(slug).first<SurveyDbRow>();
  if (!row) throw new Error("此问卷未开启结果查询");
