...
function QuestionEditor({ question, index, total, onChange, onDelete, onMove }: { question: SurveyQuestion; index: number; total: number; onChange: (question: SurveyQuestion) => void; onDelete: () => void; onMove: (offset: number) => void }) {
  ...
  return <article className="survey-question-editor">
    <header><b>第 {index + 1} 题</b><div><button type="button" disabled={index === 0} onClick={() => onMove(-1)}>上移</button><button type="button" disabled={index === total - 1} onClick={() => onMove(1)}>下移</button><button className="danger" type="button" onClick={onDelete}><Icon name="trash" />删除</button></div></header>
