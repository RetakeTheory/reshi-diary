...
type Draft = { id?: string; listType: FoodRankingType; restaurant: string; location: string; category: string; summary: string; details: string; tags: string; imageUrl: string; latitude: string; longitude: string };
const blank: Draft = { listType: "red", restaurant: "", location: "", category: "", summary: "", details: "", tags: "", imageUrl: "", latitude: "", longitude: "" };
...
export default function FoodRankingManager() {
  ...
  function edit(item: FoodRankingEntry) { setDraft({ id: item.id, listType: item.listType, restaurant: item.restaurant, location: item.location, category: item.category, summary: item.summary, details: item.details, tags: item.tags.join("、"), imageUrl: item.imageUrl, latitude: item.latitude === null ? "" : String(item.latitude), longitude: item.longitude === null ? "" : String(item.longitude) }); setMessage(""); }
  ...
  async function save(event: FormEvent) {
    ...
    try {
      const response = await fetch(draft.id ? `/api/admin/food-rankings/${draft.id}` : "/api/admin/food-rankings", { method: draft.id ? "PUT" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...draft, latitude: draft.latitude || null, longitude: draft.longitude || null, tags: draft.tags.split(/[、,，]/).map((item) => item.trim()).filter(Boolean) }) });
