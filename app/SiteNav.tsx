  ...
  return (
    <nav className="nav shell site-nav" aria-label="主导航">
      ...
      <div className="nav-links">
        ...
        <div className="nav-directory">
          ...
          <div className="nav-dropdown">
            ...
            <a href="/plugins/food-rankings"><span><Icon name="ranking" /></span><b>{copy.rankingTitle}</b><small>{copy.rankingCaption}</small></a>
          </div>
        </div>
      ...
      {backHref ? (
        ...
        <div className="desktop-nav-actions"><a className="admin-link" href="https://admin.rettheory.top/login">{copy.write} <ArrowIcon direction="up-right" /></a><a className="nav-reader-avatar" href={reader ? "/account" : "/login"} aria-label={reader ? `${reader.displayName} 的${copy.readerCenter}` : copy.readerLogin}><ReaderAvatar src={reader?.avatarUrl} name={reader?.displayName || "读者"} /></a></div>
      )}
      ...
      <div id="mobile-site-menu" className={`mobile-site-menu${open ? " is-open" : ""}`} hidden={!open}>
        ...
        <a href={backHref || "https://admin.rettheory.top/login"} onClick={() => setOpen(false)}>{backLabel || copy.write}<ArrowIcon direction={backHref ? "left" : "up-right"} /></a>
      </div>
    </nav>
  );
}
