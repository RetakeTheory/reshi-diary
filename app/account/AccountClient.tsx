...
export default function AccountClient() {
  ...
  if (!user) return <section className="account-shell shell"><div className="account-login-required"><Icon name="user" /><h1>还没有登录</h1><p>登录后可以管理头像、积分、Passkey 和工单。</p><a href="/login?next=/account">登录 / 注册</a></div></section>;
  const progress = user.level === 16 ? 100 : user.points % 100;

  return <section className="account-shell shell">
    <header className="account-head reader-profile-head"><div className="reader-profile"><ReaderAvatar src={user.avatarUrl} name={user.displayName} size={92} /><div><p>READER ACCOUNT / 读者账户</p><h1>{user.displayName}</h1><span>{user.email}</span><AvatarCropper onUploaded={(value) => setUser(value as User)} /></div></div><button type="button" onClick={logout}>退出登录</button></header>
    <section className="level-card" style={{ "--level-color": user.levelColor } as React.CSSProperties}><div><span className="level-badge">LV.{user.level}</span><div><b>{user.points} 积分</b><small>{user.level === 16 ? "已到达最高等级" : `距离 LV.${user.level + 1} 还差 ${100 - progress} 分`}</small></div></div><div className="level-progress" aria-label={`等级进度 ${progress}%`}><i style={{ width: `${progress}%` }} /></div></section>
    <section className="daily-tasks"><header><p>DAILY / 每日任务</p><h2>今天也来留个脚印</h2><span>签到、评论、回应每日各计分一次。</span></header><div>{tasks.map((task) => <article key={task.key} className={task.completed ? "is-complete" : ""}><Icon name={task.completed ? "check" : task.key === "comment" ? "comment" : task.key === "reaction" ? "heart" : "spark"} /><div><b>{task.label}</b><span>+{task.points} 积分</span></div>{task.key === "check_in" && !task.completed ? <button type="button" disabled={busy} onClick={checkIn}>签到</button> : <small>{task.completed ? "今日完成" : "去文章页完成"}</small>}</article>)}</div></section>
    <div className="account-passkeys"><div><Icon name="key" /><p>PASSKEY</p><h2>更快、更安全地回来</h2><span>Passkey 使用指纹、面容或设备 PIN；邮箱验证码仍可用于恢复。</span></div><div>{supported ? <div className="account-passkey-add"><label><span>设备名称</span><input value={name} maxLength={40} onChange={(event) => setName(event.target.value)} /></label><button type="button" disabled={busy} onClick={addPasskey}>{busy ? "正在添加…" : "添加 Passkey"}</button></div> : <p>当前浏览器不支持 Passkey。</p>}<ul>{passkeys.map((item) => <li key={item.id}><span><Icon name="check" /></span><div><b>{item.name}</b><small>添加于 {new Date(item.createdAt).toLocaleDateString("zh-CN")}</small></div></li>)}</ul>{!passkeys.length && <p className="account-passkey-empty">尚未添加 Passkey。</p>}</div></div>
    <section className="ticket-center"><header><p>TICKETS / 意见与问题</p><h2>把问题交给我</h2><span>提交后可在这里查看处理状态与回复。</span></header><div className="ticket-layout"><form onSubmit={submitTicket}><label><span>类型</span><select value={ticket.category} onChange={(event) => setTicket({ ...ticket, category: event.target.value })}><option value="feedback">意见建议</option><option value="problem">遇到问题</option><option value="question">使用咨询</option></select></label><label><span>标题</span><input maxLength={80} value={ticket.title} onChange={(event) => setTicket({ ...ticket, title: event.target.value })} required /></label><label><span>详细描述</span><textarea maxLength={2000} value={ticket.body} onChange={(event) => setTicket({ ...ticket, body: event.target.value })} required /></label><button type="submit" disabled={busy}>提交工单</button></form><div className="ticket-list">{tickets.length ? tickets.map((item) => <article key={item.id}><div><span className={`ticket-status status-${item.status}`}>{statusLabel[item.status] || item.status}</span><time>{new Date(item.updatedAt).toLocaleDateString("zh-CN")}</time></div><h3>{item.title}</h3><p>{item.body}</p>{item.adminReply && <blockquote><b>管理员回复</b>{item.adminReply}</blockquote>}</article>) : <div className="ticket-empty">还没有提交过工单。</div>}</div></div></section>
